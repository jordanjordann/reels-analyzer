...
import { useProfileAnalysis, useRefreshProfileAnalysis } from "@/api/profile-analysis/hooks";
  ...
  return (
    <div className="rounded-lg bg-muted/30 p-3 text-center">
      <p className="text-[10px] font-mono uppercase text-muted-foreground">
        {label.replace(/([A-Z])/g, " $1").trim().slice(0, 16)}
      </p>
      <p className={cn("text-2xl font-mono font-bold", scoreColor(score))}>{score}</p>
    </div>
  );
}
...
function StrengthWeaknessColumn({
  ...
  return (
    <div className="rounded-2xl border bg-background/30 p-5">
      <p className={cn("text-xs font-mono uppercase tracking-wider mb-3", color)}>{title}</p>
          ...
          return (
            <div key={i}>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-semibold">{DIMENSION_LABELS[item.dimension] ?? item.dimension}</span>
                <span className="text-xs font-mono text-muted-foreground">({item.avgScore})</span>
              </div>
...
function ProfileContent({ profile }: { profile: ProfileAnalysis & { reelCount: number; updatedAt: string } }) {
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-2xl border bg-background/30 p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(profile.personalStyle).map(([key, value]) => (
            <div key={key} className="rounded-lg border border-purple-400/20 bg-purple-400/5 p-4">
      ...
      <div className="rounded-2xl border bg-background/30 p-5">
        ...
        <div className="grid grid-cols-4 gap-3">
          <ScoreCard label="Performance" score={profile.averageScorecard.performanceScore} />
          <ScoreCard label="Creative" score={profile.averageScorecard.creativeScore} />
          <ScoreCard label="Replication" score={profile.averageScorecard.replicationScore} />
          <ScoreCard label="Viral Quality" score={profile.averageScorecard.viralQualityScore} />
        </div>
      </div>
          ...
          <div className="flex flex-col gap-3">
            {profile.viralFormulaPatterns.map((pattern, i) => (
              <div key={i} className="rounded-lg border border-accent/20 bg-accent/5 p-3">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-sm font-semibold text-accent">{pattern.formulaName}</span>
        ...
        <div className="rounded-2xl border bg-background/30 p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {profile.audiencePsychologyPatterns.map((pattern, i) => (
              <div key={i} className="rounded-lg border border-blue-400/20 bg-blue-400/5 p-3">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-semibold text-blue-400">{pattern.theme}</span>
                  <span
                    className={cn(
        ...
        <div className="rounded-2xl border bg-background/30 p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {profile.recurringRedFlags.map((flag, i) => (
              <span key={i} className="rounded-md bg-orange-400/10 px-3 py-1.5 text-xs text-orange-400">
                {flag}
              </span>
            ))}
          </div>
      ...
      {profile.replicationAdvice && (
        <div className="rounded-2xl border bg-background/30 p-5">
          ...
          <p className="text-sm text-muted-foreground">{profile.replicationAdvice}</p>
        </div>
      )}
            ...
            <div className="rounded-2xl border bg-background/30 p-5">
              ...
              <div className="flex flex-col gap-2">
                {profile.topReels.map((reel, i) => (
                  <div key={i} className="flex items-center justify-between rounded-md bg-green-400/5 px-3 py-2">
                    <span className="text-xs font-mono">{reel.shortcode}</span>
                    <span className={cn("text-sm font-mono font-bold", scoreColor(reel.score))}>
                      {reel.score}
                    </span>
                  </div>
                ))}
            ...
            <div className="rounded-2xl border bg-background/30 p-5">
              ...
              <div className="flex flex-col gap-2">
                {profile.bottomReels.map((reel, i) => (
                  <div key={i} className="flex items-center justify-between rounded-md bg-red-400/5 px-3 py-2">
                    <span className="text-xs font-mono">{reel.shortcode}</span>
                    <span className={cn("text-sm font-mono font-bold", scoreColor(reel.score))}>
                      {reel.score}
                    </span>
...
export function ProfileAnalysisTab({ username }: { username: string }) {
  ...
  const { mutate: refresh, isPending: isRefreshing } = useRefreshProfileAnalysis(username);
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-24 text-center text-muted-foreground">
        ...
        <div>
          ...
          <p className="mt-1 text-xs">Analyze at least 2 reels to see profile insights</p>
        </div>
        <button
          type="button"
          onClick={() => refresh()}
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between border-t border-border pt-4">
        <p className="text-xs text-muted-foreground">
          Based on {data.profile.reelCount} reels · Updated {formatTimeAgo(data.profile.updatedAt)}
        </p>
        <button
          type="button"
          onClick={() => refresh()}
        ...
        >
          <RefreshCwIcon className={cn("size-3.5", isRefreshing && "animate-spin")} aria-hidden="true" />
