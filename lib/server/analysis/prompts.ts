...
For improvementSuggestions, derive 3-5 specific, actionable suggestions by analyzing:
1. The LOWEST-scoring dimensions in qualityBreakdown — if hookStrength is below 7, suggest concrete hook improvements (e.g., "ganti hook dengan pola 3 detik pertama yang langsung tunjukkan hasil akhir sebelum proses").
2. The redFlags identified — each flag should map to a fix (e.g., "no_product_or_commercial_bridge" → "tambahkan CTA jelas di akhir video yang mengarahkan ke produk/layanan").
3. View-to-follower ratio — if views are low relative to followers, suggest distribution improvements (caption keywords, posting time, hashtag strategy).
4. CreativeBreakdown weaknesses — if retentionDesign is low, suggest specific editing techniques (pattern interrupts, B-roll, text overlays, zoom cuts every 3-5 seconds).
5. Duration vs engagement — if the reel is long but engagement is low, suggest cutting dead air, tightening pacing, or splitting into a series.
6. Caption strategy — if caption is weak or missing, suggest a hook-first caption structure with a clear CTA.

Each suggestion must reference the specific weakness it addresses. Do NOT give generic advice like "improve quality" or "be more engaging" — always specify WHAT to change and HOW.
