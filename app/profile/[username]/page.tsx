...
import { useProfileDetail } from "@/api/profiles/hooks";
import { ReelGrid } from "@/app/profile/[username]/components/grids/ReelGrid";
import { AnalysisModal } from "@/app/profile/[username]/components/modals/AnalysisModal";
import { NewAnalysisModal } from "@/app/profile/components/modals/NewAnalysisModal";
import { ProfileAnalysisSection } from "@/app/profile/[username]/components/sections/ProfileAnalysisSection";
...
function UserReelsContent() {
  ...
  const { data: profileData } = useProfileDetail(username);
  const reels = data?.reels ?? [];
  const profile = profileData?.profile ?? null;

  function handleReelClick(reel: AnalysisReelSummary) {
    router.push(`/profile/${username}?v=${reel.igShortcode}`);
  }
  ...
  return (
    <div className="flex min-h-dvh flex-col p-6 lg:p-8">
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="text-center">
            <p className="text-lg font-mono font-bold">
              {profile?.metadata.followerCount != null ? formatCount(profile.metadata.followerCount) : "—"}
            </p>
            <p className="text-xs text-muted-foreground">followers</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-mono font-bold">
              {profile?.metadata.followingCount != null ? formatCount(profile.metadata.followingCount) : "—"}
            </p>
            <p className="text-xs text-muted-foreground">following</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-mono font-bold">
              {profile?.metadata.postCount != null ? formatCount(profile.metadata.postCount) : "—"}
            </p>
      ...
      <div className="mt-6 flex-1">
        ...
        {activeTab === "profile" && (
          <ProfileAnalysisSection username={username} />
        )}
      </div>
